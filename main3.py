...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def setup_activity_table(self):
        self.activity_model = QSqlQueryModel()  # Alterado para QSqlQueryModel
        self.activity_model.setQuery('SELECT * FROM Activity')  # Define a query
...
class ActivityWindow(QDialog, Ui_Dialog_Activity):
    # Incluimos o activity_model no construtor para poder atualizar o modelo quando uma nova atividade for criada
    # Incluimos o parent no construtor para poder acessar o método filter_tasks_by_activity() do MainWindow, inicialmente ele é None pois não temos um parent ainda 
    # se não tivessemos incluido o activity_model no construtor, teriamos que acessar o modelo através do método parent().activity_model
